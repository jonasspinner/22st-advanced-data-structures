...
namespace ads {
    ...
    public:
        ...
        [[nodiscard]] size_type rank1(size_type i) const {
            ...
            for (size_type block_idx = 0; block_idx < max_num_blocks && block_idx < j; ++block_idx) {
        ...
        void push_front_block(block_type block) {
            ...
            size_type last_block_idx = std::min((size() - 1) / block_width, max_num_blocks - 1);
        ...
        void split_into_empty(SmallStaticBitVector &right) {
            ...
            for (size_type j = max_num_blocks / 2; j < max_num_blocks; ++j, ++k) {
        ...
        friend std::ostream &operator<<(std::ostream &os, const SmallStaticBitVector &bv) {
            os << "SmallStaticBitVector(size=" << bv.size() << ", bits=";
